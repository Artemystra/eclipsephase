...
/**
 * Positions the Armor sideCar tooltip explicitly, opening it to the right of the
 * hover anchor and vertically centred against it, and keeps it open while the user
 * hovers into the tooltip itself (e.g. to scroll its content).
 *
 * The tooltip lives inside .wrapperleft, whose overflow:hidden clips any absolutely
 * positioned child that tries to extend past the narrow left column. The CSS rule
 * therefore keeps position:fixed so the tooltip escapes that clip and can overlap the
 * wide centre column. The horizontal/vertical placement is set here as explicit pixel
 * values (rather than relying on the browser resolving a top/left:auto "static
 * position" for an out-of-flow element — behaviour that proved cross-engine unreliable).
 *
 * Because the tooltip now renders far from its anchor, plain CSS :hover breaks:
 * moving the mouse from the anchor to the tooltip crosses a gap that's neither the
 * anchor nor the tooltip, so :hover (and the display:none it gates) drops instantly,
 * before the pointer ever reaches the tooltip. A JS-managed "armor-tooltip-open" class
 * with a short close delay bridges that gap - the delay is cancelled if the pointer
 * lands on the anchor or the tooltip before it elapses.
 *
 * @param {HTMLElement} html - The rendered actor sheet root element.
 */
export function positionArmorTooltip(html) {
  const anchor = html.querySelector(".armorSideCar .tooltip");
  if (!anchor || anchor.dataset.epArmorTooltipBound) return;
  anchor.dataset.epArmorTooltipBound = "1";

  const popup = anchor.querySelector("div[role=tooltipNegative]");
  if (!popup) return;

  // Horizontal breathing room between the anchor's right edge and the tooltip.
  const GAP = 12;
  // Grace period to let the pointer travel from the anchor to the (now distant) tooltip.
  ...
  const open = () => {
    ...
    // Open beside (to the right of) the anchor.
    popup.style.left = `${rect.right + GAP}px`;
    // Anchor's vertical midpoint; the CSS transform: translateY(-50%) then shifts the
    // tooltip up by half its own height, centring it against the anchor without needing
    // to measure the tooltip's (not-yet-rendered) height here.
    popup.style.top = `${rect.top + rect.height / 2}px`;
    anchor.classList.add("armor-tooltip-open");
  };

  const scheduleClose = () => {
    if (hideTimeout) clearTimeout(hideTimeout);
    hideTimeout = setTimeout(() => {
      anchor.classList.remove("armor-tooltip-open");
      hideTimeout = null;
    }, CLOSE_DELAY);
  };
